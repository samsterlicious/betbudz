...
export class OutstandingBetsComponent implements OnInit {
  ...
  constructor(private messageService: MessageService, private auth: AuthService, private api: BackendService, private confirmationService: ConfirmationService, private spinner: SpinnerService) {
    this.spinner.turnOn();
    this.actionSubject = new BehaviorSubject(true);

    this.viewModel$ = combineLatest({
      outstanding: this.actionSubject.asObservable().pipe(mergeMap(()=>this.api.getOutstandingBets().pipe(
        tap(() => {
          this.spinner.turnOff();
        })
      ))),
      user: auth.user$.pipe(tap(user=>console.log(user)))
    })
    ...
    if(bet.personOne === email || bet.personTwo === email) {
      if(bet.winner === email) {
        ...
        this.confirmationService.confirm({
          ...
          accept: () => {
            this.spinner.turnOn();
            this.api.updateBet({recv: true, id: `${bet.week}#${bet.game}#${bet.id}`}).subscribe(
              {complete: () => {
